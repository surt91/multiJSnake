...
var FOOD = "#cc2200";
var BG_COLOR = "#000";
var SPEED = 200; // speed: 200 -> 5 steps per second

var paused = true;
var c = document.getElementById("restfulsnake");
...
var init = function init() {
  ...
  }).then(function (initial_state) {
    console.log(initial_state);
    W = initial_state.width;
    H = initial_state.height;
    ID = initial_state.id;
    c.width = W * SCALE;
    c.height = H * SCALE; // prevent scrolling so that we can use touch events of navigation
...
document.ontouchmove = function (evt) {
  ...
  var yUp = evt.touches[0].clientY; // only handle the event, if the swipe started or ended in the canvas

  var r = c.getBoundingClientRect();

  if (xUp - r.left > 0 && yUp - r.top > 0 && xUp - r.right < 0 && yUp - r.bottom < 0 || xDown - r.left > 0 && yDown - r.top > 0 && xDown - r.right < 0 && yDown - r.bottom < 0) {// we are inside, just pass
  } else {
  ...
  var xDiff = xDown - xUp;
  var yDiff = yDown - yUp; // which component is longer
...
function draw(state) {
  ...
  var _iterator = _createForOfIteratorHelper(state.snake.tail),
      _step;
  ...
  document.getElementById("score").innerHTML = "Score: ".concat(state.score);
}
...
__webpack_require__(/*! stompjs */ "./node_modules/stompjs/index.js");

function register(registrations) {
  var socket = SockJS('/dynamic');
